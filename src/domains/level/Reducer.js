...
const move = (state, x, y) => {
  ...
  const targetIsWall =
    state.getIn(['tiles', targetY, targetX]) === TileTypes.WALL
  const nextTargetIsWall =
    state.getIn(['tiles', nextToTargetY, nextToTargetX]) === TileTypes.WALL
...
const moveUp = state =>
  move(state, 0, -1).setIn(
    ['player', 'orientation'],
    PlayerOrientations.PLAYER_UP
  )

const moveDown = state =>
  move(state, 0, 1).setIn(
    ['player', 'orientation'],
    PlayerOrientations.PLAYER_DOWN
  )

const moveLeft = state =>
  move(state, -1, 0).setIn(
    ['player', 'orientation'],
    PlayerOrientations.PLAYER_LEFT
  )

const moveRight = state =>
  move(state, 1, 0).setIn(
    ['player', 'orientation'],
    PlayerOrientations.PLAYER_RIGHT
  )
